...
class TestConcordanceCorCoeff:
    ...
    def test_constant_predictions(self, ccc_loss):
        ...
        moments - not just one bad step, training never recovers and every
        later eval silently repeats the exact same (frozen) numbers. An
    ...
    def test_both_constant_and_equal(self, ccc_loss):
        """Predictions and ground truth both constant and equal: every term
        in both the correlation and the outer CCC denominator is zero."""
        predictions = torch.tensor([3.0, 3.0, 3.0, 3.0, 3.0])
        ground_truth = torch.tensor([3.0, 3.0, 3.0, 3.0, 3.0])
