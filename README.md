...
* **nkululeko.predict**: unified prediction module — predict labels for audio files, folders, CSVs or microphone input, using a feature extractor, an autopredict target (SNR, MOS, age, gender, emotion, arousal/valence, …) or a previously trained model. Replaces the former `nkululeko.demo`, `nkululeko.feature_demo` and `nkululeko.testing` modules.
