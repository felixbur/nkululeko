...
class DataFilter:
    ...
    def limit_samples(self, data_name=''):
        ...
        if max:
            if self.df.shape[0] < int(max):
                ...
                self.util.debug(f'{data_name}: limited samples to {max}, reduced samples from {self.df.shape[0]} to {df.shape[0]}')
    ...
    def limit_speakers(self, data_name=''):
        ...
        if max:
            ...
            self.util.debug(f'{data_name}: limited samples to {max} per speaker, reduced samples from {self.df.shape[0]} to {df.shape[0]}')
            self.util.copy_flags(self.df, df)   
            self.df = df
            return df
        else:
    ...
    def filter_duration(self, data_name=''):
        ...
        if data_name=='':
            ...
            min_dur = self.util.config_val_data(data_name, 'min_duration_of_sample', False)
    ...
    def filter_value(self, data_name=''):
        ...
        if filters_str:
            ...
            for f in filters:
                ...
                df = df[df[col]==val]
                post = df.shape[0]
                self.util.debug(f'{data_name}: filtered {col}={val}, reduced samples from {pre} to {post}')
            self.util.copy_flags(self.df, df)   
            self.df = df
            return df
        else:
