...
class XGB_model(Model):
    ...
    def __init__(self, df_train, df_test, feats_train, feats_test):
        ...
        self.early_stopping_rounds = self.util.config_val("MODEL", "early_stopping_rounds", False)
    ...
    def train(self):
        ...
        if self.early_stopping_rounds:
            ...
            if split3 and self.feats_test is not None and self.df_test is not None:
                ...
                self.util.debug(f"Training XGBoost with early stopping (using dev set):")
                self.util.debug(f"  - early_stopping_rounds: {self.early_stopping_rounds}")
                ...
                val_split = float(self.util.config_val("MODEL", "validation_split", 0.2))
                
                # Split training data into train and validation
                feats_train_split, feats_val, labels_train_split, labels_val = train_test_split(
                    feats, labels, test_size=val_split, random_state=42, stratify=labels
                )
                
                # Set up early stopping with validation split
                eval_set = [(feats_train_split, labels_train_split), (feats_val, labels_val)]
                fit_params['eval_set'] = eval_set
                fit_params['verbose'] = True
                ...
                self.util.debug(f"Training XGBoost with early stopping (using validation split):")
                self.util.debug(f"  - early_stopping_rounds: {self.early_stopping_rounds}")
                self.util.debug(f"  - eval_metric: {self.eval_metric}")
