...
class TestTranscriber:
    ...
    def test_init_custom_params(self, mock_load_model):
        ...
        transcriber = Transcriber(
            model_name="base", device="cpu", language="es", util=mock_util
        )
    ...
    def test_transcribe_index_with_cache(
        self, mock_isfile, mock_basename, mock_path, mock_mkdir, mock_read
    ):
        mock_util = Mock()
        ...
        index = pd.Index([("file1.wav", timedelta(seconds=0), timedelta(seconds=1))])
    ...
    @patch("nkululeko.autopredict.whisper_transcriber.audeer.basename_wo_ext")
    @patch("nkululeko.autopredict.whisper_transcriber.os.path.isfile")
    def test_transcribe_index_without_cache(
        self,
        mock_isfile,
        mock_basename,
        mock_path,
