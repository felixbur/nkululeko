...
class DummyUtil:
    ...
    def get_path(self, key):
        return "/tmp/test_store/"

    def get_exp_name(self, only_train=False):
        return "test_exp"

    def write_store(self, df, path, format):
        pass

    def get_store(self, path, format):
        return pd.DataFrame()
...
def mock_config():
    ...
    mock_config = {
        "EXP": {"root": "/tmp/test_nkululeko", "name": "test_exp"},
        "FEATS": {
        ...
        "DATA": {"needs_feature_extraction": "False"},
        "MODEL": {"n_jobs": "1"},
    }
...
def sample_data_df():
    ...
    index_tuples = [
        (audio_file, pd.Timedelta(0), pd.Timedelta(seconds=1))
        for audio_file in audio_files
    ]
    multi_index = pd.MultiIndex.from_tuples(
        index_tuples, names=["file", "start", "end"]
    )
