...
- label: 'fake' or 'real'
- speaker: speaker/file identifier (extracted from filename)
"""
...
def process_database(data_dir, output_dir):
    ...
    for split_name, split_dir in splits.items():
        ...
        # Extract speaker ID from filename (use the base filename without extensions)
        fake_df = pd.DataFrame({
            "file": [str(Path(f).relative_to(data_dir.parent)) for f in fake_files],
            "label": ["fake"] * len(fake_files),
            "speaker": [Path(f).stem.split('.')[0] for f in fake_files]  # Use first part of filename
        })
        
        real_df = pd.DataFrame({
            "file": [str(Path(f).relative_to(data_dir.parent)) for f in real_files],
            "label": ["real"] * len(real_files),
            "speaker": [Path(f).stem.split('.')[0] for f in real_files]  # Use first part of filename
        })
